...
@app.route('/resources/install', methods=['POST', 'OPTIONS'])
def resources_install():
    """Install selected resources to a VastAI instance"""
    if request.method == 'OPTIONS':
        return ("", 204)
    ...
    ui_home = data.get('ui_home')
    resource_paths = data.get('resources', [])
    
    if not ssh_host or not ui_home:
        return jsonify({
            'success': False,
            'message': 'ssh_host and ui_home are required'
        }), 400
    
    if not resource_paths:
        return jsonify({
            'success': False,
            'message': 'At least one resource must be selected'
        }), 400
    
    # Generate installation job ID
    import uuid
    job_id = str(uuid.uuid4())
    
    try:
        # Get resources to install
        resources_to_install = []
        for path in resource_paths:
            resource = resource_manager.get_resource(path)
            if resource:
                resources_to_install.append(resource)
            else:
                logger.warning(f"Resource not found: {path}")
        
        if not resources_to_install:
            return jsonify({
                'success': False,
        ...
        def progress_callback(progress_data):
            ...
            if socketio:
                try:
                    ...
                    socketio.emit(
                        'resource_install_progress',
                        progress_data,
                        namespace='/resources',
                        room=job_id
                    )
                    logger.debug(f"Emitted progress: {progress_data}")
                except Exception as e:
                    logger.error(f"Failed to emit progress: {e}")
        
        # Create installer with progress callback
        from ..resources import ResourceInstaller
        installer = ResourceInstaller(progress_callback=progress_callback)
        ...
        def install_thread():
            try:
                result = installer.install_multiple(
                    ssh_host=ssh_host,
                    ssh_port=ssh_port,
                    ui_home=ui_home,
                    resources=resources_to_install
                )
                
                # Send completion event
                if socketio:
                    socketio.emit(
                        'resource_install_complete',
                        result,
                        namespace='/resources',
                        room=job_id
                    )
                
                logger.info(f"Installation job {job_id} completed: {result['success']}")
                
            except Exception as e:
                logger.error(f"Installation job {job_id} failed: {e}")
                if socketio:
                    socketio.emit(
                        'resource_install_error',
        ...
        return jsonify({
            ...
            'resource_count': len(resources_to_install)
        })
